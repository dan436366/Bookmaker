...
public class BettingAgencyTests {
    ...
    @Test
    void testPlaceBet() {
        when(mockUser.getBalance()).thenReturn(100.0); // Якщо getBalance() повертає double
        doNothing().when(mockUser).placeBet(50);

        mockUser.placeBet(50);
        verify(mockUser).placeBet(50); // перевірка, що метод placeBet викликаний
        assertEquals(100, mockUser.getBalance(), "Balance should decrease after placing a bet");
    }

    @Test
    void testWinBet() {
        when(mockUser.getBalance()).thenReturn(50.0); // Якщо getBalance() повертає double
        doNothing().when(mockUser).winBet(30);

        mockUser.winBet(30);
        verify(mockUser).winBet(30); // перевірка, що метод winBet викликаний
        assertEquals(80, mockUser.getBalance(), "Balance should increase after winning a bet");
    }

    @Test
    void testCanPlaceBet() {
        when(mockMatch.canPlaceBet(100.0, 50.0)).thenReturn(true);
        assertTrue(mockMatch.canPlaceBet(100.0, 50.0), "User should be able to place the bet with sufficient balance");
    }

    @Test
    void testCantPlaceBet() {
        when(mockMatch.canPlaceBet(100.0, 101.0)).thenReturn(false);
        assertFalse(mockMatch.canPlaceBet(100.0, 101.0), "User should not be able to place the bet with insufficient balance");
    }
}